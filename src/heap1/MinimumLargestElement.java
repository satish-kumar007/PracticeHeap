...
public class MinimumLargestElement {
    ...
    public static int solve(int[] originalArr, int B) {
        ...
        for (int i = 0; i < n; i++){
            pq.offer(new Pair(2 * originalArr[i], i));
        }
        ...
        while(B-- > 0) {
            ...
            System.out.println(top+ " ===== "+top.index+" ==== "+top.value);
            newArr[top.index] = top.value;
            pq.offer(new Pair(originalArr[top.index] + top.value, top.index));
        }
        int mx = newArr[0];
        for (int i = 0; i < n; i++) mx = Math.max(mx, newArr[i]);
        return mx;
    }
}
